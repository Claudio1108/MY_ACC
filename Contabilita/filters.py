...
class ClienteFilter(django_filters.FilterSet):
    nominativo = django_filters.CharFilter(
        label="Nominativo", field_name="nominativo", lookup_expr="icontains"
    )
...
class ProtocolloFilter(django_filters.FilterSet):
    ...
    pratica = django_filters.CharFilter(
        label="Pratica", field_name="pratica", lookup_expr="icontains"
    )
    data_consegna = django_filters.BooleanFilter(
        label="Data Consegna", field_name="data_consegna", lookup_expr="isnull", exclude=True
    )
...
class ConsulenzaFilter(django_filters.FilterSet):
    ...
    attivita = django_filters.CharFilter(
        label="Attività", field_name="attivita", lookup_expr="icontains"
    )
    data_consegna = django_filters.BooleanFilter(
        label="Data Consegna", field_name="data_consegna", lookup_expr="isnull", exclude=True
    )
...
class SpesaGestioneFilter(django_filters.FilterSet):
    ...
    fattura = django_filters.CharFilter(
        label="Fattura", field_name="fattura", lookup_expr="icontains"
    )
    causale = django_filters.CharFilter(
        label="Causale", field_name="causale", lookup_expr="icontains"
    )
...
class SpesaCommessaFilter(django_filters.FilterSet):
    ...
    protocollo_exist = django_filters.BooleanFilter(
        label="Esistenza Protocollo", field_name="protocollo", lookup_expr="isnull", exclude=True
    )
    protocollo_id = django_filters.CharFilter(
        label="Protocollo (id)", field_name="protocollo__identificativo", lookup_expr="icontains"
    )
    protocollo_address = django_filters.CharFilter(
        label="Protocollo (indirizzo)", field_name="protocollo__indirizzo", lookup_expr="icontains"
    )
    importo = django_filters.RangeFilter(label="Importo (min-max) €", field_name="importo")
