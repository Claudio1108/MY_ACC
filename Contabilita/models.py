...
class Ricavo(models.Model):
    ...
    movimento = models.CharField(
        max_length=15, choices=(("ACCONTO", "ACCONTO"), ("SALDO", "SALDO")), default="ACCONTO"
    )
    ...
    # foreign_key
    intestatario_fattura = models.ForeignKey(
        Socio, on_delete=models.CASCADE, related_name="sociofatturaricavo", null=True, blank=True
    )
    protocollo = models.ForeignKey(
        Protocollo, on_delete=models.CASCADE, related_name="ricavi", null=True, blank=True
    )
    note = RichTextField(null=True, blank=True)
    destinazione = models.CharField(
        max_length=15, choices=(("DEPOSITO", "DEPOSITO"), ("CARTA", "CARTA")), default="DEPOSITO"
    )
...
class SpesaCommessa(models.Model):
    ...
    # foreign_key
    protocollo = models.ForeignKey(
        Protocollo, on_delete=models.CASCADE, related_name="spesecommessa", null=True, blank=True
    )
    note = RichTextField(null=True, blank=True)
